...
const allUpperCase = (arr) => {
  const returnArray = [];
  arr.forEach((element) => returnArray.push(element.toUpperCase()));
  return returnArray;
};
...
const fizzbuzz = (arr) => {
  const output = [];
  arr.forEach(item => {
    if(item%3 === 0 && item%5 === 0){ output.push('Fizz Buzz');}
    else if(item%3 === 0) {output.push('Fizz');}
    else if(item%5 === 0) {output.push('Buzz');}
    else{
      output.push(item);
