...
const longestString = (arr) => {
  ...
  let largest = 0;
  let idx = -1;
  while(arr.length > endLoopCount){
    if(arr[endLoopCount].length > largest){
      largest = arr[endLoopCount].length;
      idx = endLoopCount;
    }
    endLoopCount++;
  }
...
const findHappiness = (arr) => {
  let newArr = [];
  arr.map(string => string.includes(':)') ? newArr.push(string): string);
  return newArr;
};
